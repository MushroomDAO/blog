 ...
 * （避免为限速这一个小功能单独建一个 namespace），key 加 "ratelimit:" 前缀跟
 ...
 * 实际能达到的请求量比"15 分钟 5 次"这个名义上限更宽松。在"单一共享密码 + 低访问量
 * 个人博客"这个威胁模型下可接受，真正需要更强限速时应换 Durable Objects 或 Cloudflare
 * 原生的 Rate Limiting binding。
 */
 ...
 * 每次调用都会把这次尝试计入次数（不管最终登录成不成功）——限的是"打了多少次"，
 * 不是"打错了多少次"，避免攻击者用"每次都故意在最后一步失败"之类的手法绕过。
 */
async function checkAndIncrement(kv, ip) {
