 ...
 * 计费"（见 architecture.md 核心判断 7），但真正挡刷量的从来是下面这道按 IP 的限速，
 * 不是登录本身——登录只是"要不要让陌生人知道/用到这个功能"的门槛，对个人博客这个访问量级，
 * 站长权衡后决定公开语义检索（跟一直公开的 Pagefind 关键词搜索对齐），限速继续按原有强度
 * 生效。**同一套登录系统（`_lib/auth.js`/`search-auth.js`）仍然保留**，只是不再是这个
