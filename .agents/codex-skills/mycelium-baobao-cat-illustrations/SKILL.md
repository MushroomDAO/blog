---
name: mycelium-baobao-cat-illustrations
description: 生成 Mycelium「Baobao」白色德文卷毛猫 IP 风格的中文正文配图。【科普类别】文章默认用此 skill。用于为中文文章、帖子、博客、Notion 文档、工作流、方法论、流程、结构、状态、隐喻或观点生成“怪诞”“德文猫”“Baobao”“科普”“手绘”“正文配图”“文章插图”“配图建议”“shot list”“去标题/改图”等任务；视觉为纯白手绘、少量红橙蓝批注、简洁清爽，主角是白色德文卷毛猫「Baobao」。
---

# Mycelium Baobao（德文猫）怪诞正文配图

> 用途分工：本 skill 用于**科普类别**文章。科技类别用 `mycelium-mushroom-illustrations`（蘑菇小M），社会/创作思考类别用 `mycelium-avatar-illustrations`（卡通小人小J）。

## 核心定位

为中文文章设计和生成 16:9 横版正文配图。目标不是做商业插画、PPT 信息图或可爱卡通，而是把文章里的关键判断、流程、结构、状态或隐喻，变成一张清爽、怪诞、有创意、可读但不说明书的手绘解释图。

默认视觉 IP 是“Baobao”（白色德文卷毛猫）：**白色填充 + 黑色线稿轮廓**（不是黑色实心），超大尖耳、楔形小脸、瘦长身、短卷毛，好奇专注但 deadpan。Baobao必须参与画面的核心动作（拆解原理、观察、讲解），不能只是站在旁边当装饰。完整形象见 `references/baobao-ip.md`。
...
- `references/baobao-ip.md`：Baobao IP 的形象、性格、动作库和禁忌。
...
不要复刻过往案例。案例只提供风格密度和Baobao参与方式，不能直接复用“传送带断点 / Baobao拉线 / 素材鱼 / 盖章工具箱 / 常见坑路径”等已有构图，除非用户明确要求复刻某张图。每次都要从当前文章重新发明一个奇怪但成立的隐喻。
