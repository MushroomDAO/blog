...
> 维护记录：
> - 创建时间: 2026-04-07
> - 最后更新: 2026-04-11 - 添加中英文双语版本发布功能；添加双语布局支持（仅 Blog）
...
1. ✅ 分别提取中英文标题
2. ✅ 将内容用 `<!--EN-->` 标记分割
3. ✅ 左侧显示中文，右侧显示英文（双栏布局）
4. ✅ 构建并部署到 Production
5. ✅ **仅发布 Blog**（中英文双语版本不发布公众号）

**说明**：中英文双语版本采用左右分栏布局，中文在左、英文在右，仅在 Blog 发布，不生成微信公众号草稿。
...
|------|--------|------|
| 宽度 | 1200px | 保证在高清屏下清晰 |
| 高度 | 630px | 约 1.91:1 比例，适配大多数平台 |
| 格式 | JPG / WebP | JPG 兼容性最好 |
| 大小 | ~100KB | 平衡质量与加载速度 |

**设计建议：**
- 主要内容放在中心区域（安全区 800×400），边缘可能被裁剪
- 使用 `-gravity North` 时，上半部分更重要
- 避免在底部放关键信息（可能被标题/按钮遮挡）

### 文章内图片

**推荐尺寸：最大 1200px 宽，高度自适应**

| 属性 | 建议值 | 说明 |
|------|--------|------|
| 最大宽度 | 1200px | 超出会被缩放 |
| 高度 | 自适应 | 保持原始比例 |
| 格式 | JPG / WebP | 照片用 JPG，图表用 PNG |
| 大小 | ~100KB | 单张不超过 120KB |

**设计建议：**
- 不裁剪，保持完整内容
- 截图建议 1200×800 或 1200×600（2:3 或 2:1 比例）
- 文字类图片确保字体足够大（移动端可读）
...
用户需要提供：
1. **中文内容**（前半部分）
2. **英文翻译**（后半部分，用分隔线或说明标识）

### 实现原理

Blog 的 `BlogPost.astro` 布局会自动识别 `<!--EN-->` 标记：
- 标记前：中文内容（左侧栏）
- 标记后：英文内容（右侧栏）
- 无标记：单栏中文布局
...
```bash
# Step 1: 创建双语 Markdown（在中文内容后添加 <!--EN-->，然后是英文内容）

# Step 2: 构建
pnpm build

# Step 3: 部署（仅 Blog，不发布公众号）
npx wrangler pages deploy dist --project-name=blog-mushroom --branch=main
```
...
category: 'Research'
tags: ['tag1', 'tag2']
heroImage: '../../assets/images/cover-slug.jpg'
---
...
1. **仅发布 Blog**：中英文双语版本不生成微信公众号草稿
2. **内容对应**：中英文内容建议段落结构对应，便于对照阅读
3. **图片**：双语版本共用一套图片（放在中文内容中）
4. **布局响应式**：
   - 桌面端：左右双栏（中文左，英文右）
   - 移动端：上下堆叠（中文上，英文下）
...
| 2026-04-11 | 添加中英文双语版本发布功能：支持左右分栏双语布局，仅发布 Blog |
