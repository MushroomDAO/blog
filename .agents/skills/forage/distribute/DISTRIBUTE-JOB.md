...
**格式约定（8042 评审台的「英文分发草稿」页按这个切段、生成复制按钮和预填发帖链接）**：
每一段用单独一行的 `**标签**` 开头。HN 用 `**Title（首选）**`、`**URL**`、`**可选首条评论**`；
Reddit 用 `**建议版块：r/<name>**`、`**Title**`、`**Body**`，并且 **Body 必须是最后一段**，
正文一直写到文件结尾（正文里可以有加粗小标题）。dev.to 整个文件就是可粘贴的文章。
