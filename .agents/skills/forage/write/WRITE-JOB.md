...
- **daily-crawler 条目**（src=`daily-crawler`，标题形如 `[2026-09-12 S1] …`）是 Codex 的 SME AI 日报构想，
  不是一手源。把卡片上的「原始报道」「候选开源项目」原样交给子 agent，并说明：
  - 先打开原始报道核实日报的说法（日期、数字、谁说的），再读候选仓库的完整 README 确认它真在做这件事；
  - 按卡片「缺口」一栏定文章类型：有仓库 → 项目拆解；只有报道 → 行业/产品观察；两样都没有 → `write:false`；
  - 日报里的「可开源组件 / 可收费服务」是产品构想，不能当事实写进文章；要用只能作为作者观点，并写明是构想。
