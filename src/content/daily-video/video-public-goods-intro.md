...
1. **它解决什么痛点**——对应[🧠 痛点地图](/my/painpoints/)上的哪一条，不是「我们想做」而是「这里确实卡住了」
2. **为什么是自己做的**——[🔬 技术实验](/video/?cat=Tech-Experiment)那边已经开盒过现成方案，说明为什么它们不够用
...
1. **What pain it solves** — which entry on the [🧠 Pain Point Map](/my/painpoints/) it maps to; not "we felt like building it" but "people are genuinely stuck here"
2. **Why we built it ourselves** — the [🔬 Tech Experiments](/video/?cat=Tech-Experiment) line already tore down the existing options, so we explain why they fell short
...
This line is the **outlet** for the other two: [🧠 Problem Thinking](/video/?cat=Problem-Thinking) confirms a real problem, [🔬 Tech Experiments](/video/?cat=Tech-Experiment) shows the existing options fall short — so we build the missing piece and open-source it.
